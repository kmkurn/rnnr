...
from rnnr import attach_pbar_on
...
def test_attach_on(runner):
    ...
    attach_pbar_on(runner, tqdm_cls=mock_tqdm_cls)
...
def test_custom_update_size(runner):
    ...
    mock_tqdm_cls = MagicMock(spec=tqdm)
    attach_pbar_on(runner, tqdm_cls=mock_tqdm_cls, update_size=lambda s: len(s['batch']))
...
def test_display_stats(runner):
    ...
    batch_fn = lambda x: x**2
    mock_tqdm_cls = MagicMock(spec=tqdm)
    attach_pbar_on(runner, tqdm_cls=mock_tqdm_cls, stats=lambda s: {'loss': s['output']})
    runner.run(batch_fn, batches)
    assert mock_tqdm_cls.return_value.set_postfix.mock_calls == [
...
def test_with_kwargs(runner):
    ...
    attach_pbar_on(runner, tqdm_cls=mock_tqdm_cls, **kwargs)
    runner.run(Mock(), batches)
